...
public class U {
    ...
    public static String foggy(String param, int start, int end) {
        ...
        String head = param.substring(0, start);
        String middle = param.substring(start, end).replaceAll("[0-9]", "*");
        String tail = param.substring(end);
        return head + SPACE + middle + SPACE + tail;
    }
    ...
    public static String getPrefix(String image) {
        return (isNotBlank(image) && image.contains("."))
                ? image.substring(0, image.lastIndexOf(".")) : EMPTY;
    }
